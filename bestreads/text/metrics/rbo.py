...
RBO.res.__doc__ = ("Residual corresponding to min; min + res is an upper bound"
                   + " estimate")
...
def rbo_min(list1, list2, p, depth=None):
    ...
    sum_term = sum(
        p ** d / d * (overlap(list1, list2, d) - x_k)
...
def rbo_res(list1, list2, p):
    ...
    len_s, len_l = len(S), len(L)  # nopep8
    x_l = overlap(list1, list2, len_l)
    # since overlap(...) can be fractional in the general case of ties and f
    # must be an integer --> math.ceil()
    f = int(math.ceil(len_l + len_s - x_l))
    # upper bound of range() is non-inclusive, therefore + 1 is needed
    term1 = len_s * sum(p ** d / d for d in range(len_s + 1, f + 1))
    term2 = len_l * sum(p ** d / d for d in range(len_l + 1, f + 1))
    term3 = x_l * (math.log(1 / (1 - p)) - sum(p ** d / d
                                               for d in range(1, f + 1)))
    return (p ** len_s + p ** len_l - p ** f - (1 - p)
            / p * (term1 + term2 + term3))
...
def rbo_ext(list1, list2, p):
    ...
    """
    S, L = sorted((list1, list2), key=len)
    len_s, len_l = len(S), len(L)  # noqa
    x_l = overlap(list1, list2, len_l)
    x_s = overlap(list1, list2, len_s)
    ...
    sum1 = sum(p ** d * agreement(list1, list2, d)
               for d in range(1, len_l + 1))
    sum2 = sum(p ** d * x_s * (d - len_s) / len_s / d
               for d in range(len_s + 1, len_l + 1))
    term1 = (1 - p) / p * (sum1 + sum2)
    term2 = p ** len_l * ((x_l - x_s) / len_l + x_s / len_s)
    return term1 + term2
